head.ready(function () {
  ...
  _setup.ls = function () {
    ...
    $(".ht-search-form input.search-input-text").attr(
      "placeholder",
  ...
  _setup.catalog = function () {
    ...
    if (inited) {
      HT.update_status(
        "Search will use the catalog index; use Shift + Tab to limit your search to a selection of fields."
      );
    }
  };
  ...
  $("body").on(
    "change",
    '.ht-search-form .search-target input[type="radio"]',
    function (e) {
      target = this.value;
      _setup[target]();
  ...
  $action.on("click", function () {
    ...
    $content
      .find("#option-full-text-search")
      .attr("id", "option-full-text-search-2");
    $content
      .find("#option-catalog-search")
      .attr("id", "option-catalog-search-2");
    $content
      .find("label.search-label-full-text")
      .attr("for", "option-full-text-search-2");
    $content
      .find("label.search-label-catalog")
      .attr("for", "option-catalog-search-2");
    $content
      .find("label[for='global-search-ft']")
      .attr("for", "global-search-ft-2");
  ...
  var _resizer = function () {
    ...
    if (
      $header.is(":visible") &&
      active &&
      active.modal.getAttribute("id") == "search-modal"
    ) {
      bootbox.close();
    } else {
      /* NOP */
